            ...
            if(line_data_ready)
            {
                ...
                else
                {
                    ...
                    // 阿克曼差速系数：直道 / 弯道第一阶段 / 弯道后期 三档独立
                    {
                        ...
                        {
                            // ---- 弯道后期：适度差速，平衡过弯与速度 ----
                            float gain_angle = 0.0f + 0.010f * (abs_angle - 3.0f) * (abs_angle - 3.0f);
                            float gain_speed = 0.3f + 0.025f * v_target;
                            raw_gain = 0.5f * gain_angle + 0.5f * gain_speed;
                        }
